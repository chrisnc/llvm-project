; RUN: llc --force-dwarf-frame-section --exception-model=arm %s -o - --target-abi=aapcs16 -float-abi=soft | FileCheck %s
; RUN: llc --filetype=obj %s --exception-model=arm -o - --target-abi=aapcs16 -float-abi=soft | llvm-readelf -s --unwind - | FileCheck %s --check-prefix=UNWIND
